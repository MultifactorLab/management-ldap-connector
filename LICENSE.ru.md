# Лицензионное соглашение на использование компонента «MGM.LDAP.Connector»
...
1.1. Компонент – программа для ЭВМ «MGM.LDAP.Connector» (как в целом, так и ее компоненты, и функции).

1.2 ООО «МУЛЬТИФАКТОР» (далее — «Лицензиар») предлагает использовать компонент «MGM.LDAP.Connector» (далее — «Компонент»), доступный по адресу: https://github.com/MultifactorLab/management-ldap-connector по правилам и в пределах условий настоящей лицензии (далее – «Лицензия»).

1.3. Компонент представляет собой программу для ЭВМ - компонент «MGM.LDAP.Connector», позволяющую реализовать следующие программные интерфейсы: аутентификация пользователей через каталог LDAP/Active Directory; синхронизация учётных записей из LDAP-каталога в платформу Multifactor (MGM); управление параметрами подключения к LDAP и настройками синхронизации через веб-интерфейс администратора.
...
3.1.1. Применять Компонент по прямому функциональному назначению, описанному в п. 1.3 настоящей Лицензии, в целях чего производить его скачивание (копирование) и воспроизведение на устройствах Пользователя под наименованием «MGM.LDAP.Connector». Пользователь вправе использовать Компонент на неограниченном количестве устройств.
...
6.1. Условия настоящей Лицензии могут изменяться Лицензиаром в одностороннем порядке без предварительного уведомления Пользователя. Действующая версия Лицензии постоянно доступна по адресу https://github.com/MultifactorLab/management-ldap-connector/blob/main/LICENSE.ru.md.
